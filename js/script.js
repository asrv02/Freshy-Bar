const API_URL = "https://bejewled-plucky-cause.glitch.me/";
...
const calculateTotalPrice = (form, startPrice) => {
    ...
    if (Array.isArray(data.ingredietns)) {
        data.ingredietns.forEach(item => {
            totalPrice += price[item] || 0;
        })
    } else {
        totalPrice += price[data.ingredietns] || 0;
    }
...
const calculateMakeYourOwn = () => {
    ...
    const handlerChange = () => {
        ...
        if(data.ingredietns) {
            const ingredietns = Array.isArray(data.ingredietns)
            ? data.ingredietns.join(', ')
            : data.ingredietns;

            makeInputTitle.value = `Конструктор: ${ingredietns}`;
...
const createCartItem = (item) => {
    ...
    li.innerHTML = `
    <img class="order__img" src="img/make-your-own.jpg" alt="${item.title}">
    ...
    <button class="order__item-delete" data-idls="${item.idls}" aria-label="Удалить коктейл из корзины"></button>
